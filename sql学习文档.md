...
- **format**：你希望输出的格式模板（由特殊的“%”占位符组成）。



#### 2. 常用占位符“秘密清单”

这是 `DATE_FORMAT` 的核心，记住这些符号，你就掌握了变换的密码：

#### **年份 (Year)**

- **`%Y`**：4 位数字年份（如 **2025**）—— **最常用**。
- **`%y`**：2 位数字年份（如 **25**）。

#### **月份 (Month)**

- **`%m`**：2 位数字月份（**01** 到 **12**）—— **最常用**。
- **`%c`**：1 或 2 位数字月份（**1** 到 **12**）。
- **`%M`**：月份全称（如 **October**）。
- **`%b`**：月份缩写（如 **Oct**）。

#### **日期 (Day)**

- **`%d`**：2 位数字日期（**01** 到 **31**）—— **最常用**。
- **`%e`**：1 或 2 位数字日期（**1** 到 **31**）。
- **`%W`**：星期几全称（如 **Wednesday**）。
...
假设我们有一个日期：`'2025-10-15 14:30:00'`

| **SQL 语句**                              | **输出结果**             | **说明**               |
| ----------------------------------------- | ------------------------ | ---------------------- |
| `DATE_FORMAT(date, '%Y-%m-%d')`           | **2025-10-15**           | 标准的“年-月-日”       |
| `DATE_FORMAT(date, '%Y/%m')`              | **2025/10**              | **做月报汇总时最常用** |
| `DATE_FORMAT(date, '%M %d, %Y')`          | **October 15, 2025**     | 美式日期格式           |
| `DATE_FORMAT(date, '%Y年%m月%d日 %H:%i')` | **2025年10月15日 14:30** | 中文友好格式           |
| `DATE_FORMAT(date, '%W')`                 | **Wednesday**            | 只要星期几             |



### 快捷提取 (YEAR / MONTH / DAY)

如果你只需要年份或月份，用 `DATE_FORMAT(date, '%m')` 其实有点大材小用了，而且写起来慢。直接用这些专门的函数：

- **`YEAR(date)`** $\rightarrow$ 返回 **2025**
- **`MONTH(date)`** $\rightarrow$ 返回 **10**
- **`DAY(date)`** $\rightarrow$ 返回 **15**
- **`WEEKDAY(date)`** $\rightarrow$ 返回 **0-6**（0 是周一，以此类推）

> **场景：** 统计每年、每月的业务量，直接 `GROUP BY MONTH(date)` 比 `DATE_FORMAT` 性能更好一点点。



### 日期加减： DATE_ADD / DATE_SUB

这是做“留存率”或“到期提醒”最核心的功能。

- **`DATE_ADD(date, INTERVAL 7 DAY)`**：7 天后。
- **`DATE_SUB(date, INTERVAL 1 MONTH)`**：1 个月前。
...
### 计算差距：隔了多久 (DATEDIFF / TIMESTAMPDIFF)

这是面试和报表中的高频需求。

- **`DATEDIFF(end, start)`**：**只算天数**。
  - `DATEDIFF('2025-10-20', '2025-10-15')` $\rightarrow$ 返回 **5**。
- **`TIMESTAMPDIFF(unit, start, end)`**：**万能差异函数**，可以算秒、分、小时、天、周、月、年。
  - `TIMESTAMPDIFF(HOUR, '2025-10-15 08:00:00', '2025-10-15 12:00:00')` $\rightarrow$ 返回 **4**。



### 字符串转日期：(STR_TO_DATE)

有时候你导入的 Excel 数据是 `10/15/2025` 这种乱七八糟的格式，数据库没法直接进行日期运算。

- **用法：** `STR_TO_DATE('15-10-2025', '%d-%m-%Y')`
- 它会把这个字符串“翻译”成标准的数据库日期 `2025-10-15`。



### 获取当前时间：现在的坐标

- **`CURDATE()`**：今天的日期（2026-03-04）。
- **`NOW()`**：现在的精确时间（2026-03-04 18:25:10）。
