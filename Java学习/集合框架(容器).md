...
> 内存利用率：
>
> **`ArrayDeque` (精装房)**： 它底层是数组。如果你存 100 个整数，它基本就只占 100 个整数的空间（加上一点预留空位）。由于是连续内存，数据挤得很紧凑。
>
> **`LinkedList` (烂尾楼)**： 它每存一个数据，都要额外打包成一个 **`Node`（节点）对象**。
>
> - 一个 `Node` 对象包含：**数据本身** + **指向前的指针** + **指向后的指针** + **对象头（Object Header）**。
>
> CPU 缓存友好度:
>
> **`ArrayDeque` (数组)**： 数据在内存里是**连续**挨着的。CPU 每次去内存取数据时，会顺便把后面的一块数据也加载到高速缓存（Cache）里。这意味着当你处理完第 1 个，第 2 个已经在 CPU 缓存里等着你了。
>
> GC（垃圾回收）压力:
>
> 在高性能 Java 应用中，最怕的就是频繁触发 GC。
>
> - **`ArrayDeque`**： 它只在扩容时申请大块数组。平时 `add` 和 `poll` 只是在修改数组里的值，**不产生新对象**。
> - **`LinkedList`**： **每 `add` 一个元素就要 `new` 一个 `Node`，每 `poll` 一个元素就要销毁一个 `Node`。** 如果你每秒处理几万个任务，`LinkedList` 会产生海量的“短命对象”，让 JVM 的垃圾回收器忙得满头大汗，导致程序卡顿。
...
> 关于循环数组的具体实现可以重点看算法篇的循环数组,不过后面的内容也很有价值,且说的清楚
...
<img src="../assets/javaAssets/15.collection20.png" width="60%" style="display: block; margin: 0 auto;">
...
> 这里用到了位运算,原理和我们取模是一样的,只不过位运算要实现类似效果必须要求**数组的长度（length）必须是 2 的幂次方**（比如 8, 16, 32, 64...）,所以这一点和我们在算法篇讨论的自定义循环数组还不一样,不过位运算是更优雅 更性能的的一种方案
>
> **`%`（取模/除法）**：在 CPU 层面是非常昂贵的指令，需要几十个时钟周期。
>
> **`&`（位运算）**：是 CPU 的本能，只需要 **1 个时钟周期**。
...
<img src="../assets/javaAssets/15.collection21.png" width="60%" style="display: block; margin: 0 auto;">
...
> `arraycopy`是原生调用CPU寻址的命令,比我们在算法篇的CycleArray要快很多,而且为了避免出现"断开"的情况,这里是分了两次来完成的,一半是head到物理末尾,另一半是开头到结尾
>
> assert这里是断言,仅限于开发者自我约束,未来上线后会消失,追求高性能
...
<img src="../assets/javaAssets/15.collection22.png" width="60%" style="display: block; margin: 0 auto;">
...
因为`ArrayDeque` 的底层实现是数组，而 `LinkedList` 的底层实现是链表。数组是一段连续的内存空间，而链表是由多个节点组成的，每个节点存储数据和指向下一个节点的指针。因此，在使用 `LinkedList` 时，需要频繁进行内存分配和释放，而 `ArrayDeque` 在创建时就一次性分配了连续的内存空间，不需要频繁进行内存分配和释放，这样可以更好地利用 CPU 缓存，提高访问效率。
