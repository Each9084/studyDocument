...
<span style="color:teal">我的理解:</span>

> ### 1. VSM 这种方法的优缺点
>
> 这种用 0 和 1 来代表词是否存在的方法，是机器学习的“第一代”做法：
>
> | **优点**                                                 | **缺点**                                                 |
>| -------------------------------------------------------- | -------------------------------------------------------- |
> | **简单直接**：电脑处理速度快，容易实现。                 | **维度灾难**：如果词汇表有 10 万个词，向量就会长到离谱。 |
>| **解释性强**：看到哪个位置是 1，就知道这句话里有什么词。 | **语义缺失**：它只知道词“出现过”，但不知道词的“意        |
> 
> ### 2. 承上启下：从 VSM 到 Embedding
> 
> 这段内容其实是想告诉：**传统的 VSM 方法太“浪费”且太“笨”了。**
>
> 现在的深度学习（比如大模型）通常会把这种 16,384 维的稀疏向量，压缩成一个长度只有 256 或 512 的**稠密向量**（里面全是像 `0.12, -0.55` 这样的小数）。这种压缩后的向量才叫真正的 **Word Embedding（词嵌入）**。
>
> **打个比方：**
>
> - **VSM（你读到的）**：像是给每个人建一栋楼，这栋楼里只住一个人，剩下的房间都空着（稀疏）。
>- **Word Embedding（现代方法）**：像是给每个人一个身份证号，几位数字就能代表一个人的所有特征（稠密）。
...
> #### 常见 N 值的具体含义
>
> 根据你选择的 $N$ 的大小，模型被赋予了不同的名字。我们以预测“我 喜欢 吃 苹果”中最后的“苹果”为例：
>
> **① Unigram ($N=1$, 一元模型)**
>
> - **假设：** 词与词之间完全独立，下一个词的出现和前面的词**毫无关系**（$N-1 = 0$）。
> - **计算“苹果”的概率：** 只看“苹果”这个词在语料库里出现的频率。
> - **公式：** $P(\text{苹果})$
> - **缺点：** 完全丢失了语序。把一篇文章打碎成一堆乱七八糟的词，概率也是一样的。
>
> **② Bigram ($N=2$, 二元模型)**
>
> - **假设：** 当前词只和它的**前一个词**有关（$N-1 = 1$）。
> - **计算“苹果”的概率：** 在前面是“吃”的条件下，“苹果”出现的概率。
> - **公式：** $P(\text{苹果} \mid \text{吃})$
> - **大白话：** 只要看到“吃”，后面跟着“苹果”的概率就很高；看到“喝”，后面跟着“苹果”的概率就很低。
>
> **③ Trigram ($N=3$, 三元模型)**
>
> - **假设：** 当前词只和它的**前两个词**有关（$N-1 = 2$）。
> - **计算“苹果”的概率：** 在前面是“喜欢 吃”的条件下，“苹果”出现的概率。
> - **公式：** $P(\text{苹果} \mid \text{喜欢, 吃})$
...
> 对于上述的理解
>
> #### 1. 核心问题：如何用数学衡量一句话顺不顺？
>
> 语言模型的核心任务，是计算**一句话出现的概率**。
>
> 比如，我们要计算“我 喜欢 吃 苹果”这句话在汉语中出现的概率 $P(\text{我, 喜欢, 吃, 苹果})$。
>
> 根据概率论的**链式法则**，这句话的概率应该等于：
>
> $$P(\text{我, 喜欢, 吃, 苹果}) = P(\text{我}) \times P(\text{喜欢} \mid \text{我}) \times P(\text{吃} \mid \text{我, 喜欢}) \times P(\text{苹果} \mid \text{我, 喜欢, 吃})$$
>
> 也就是说，要预测“苹果”，必须考虑前面所有的词（我、喜欢、吃）。如果一句话有 100 个词，预测第 100 个词就要考虑前面 99 个词。**在数学上，这会导致参数量爆炸，根本无法计算。**
>
> #### 2.拯救者：马尔可夫假设（Markov Assumption）
>
> 为了解决这个“历史包袱太重”的问题，数学家马尔可夫提出了一个著名的假设：
>
> > **“未来的状态只取决于当前或有限的过去，而与久远的过去无关。”**
>
> 把这个假设应用到语言里，就是 **N-gram 模型** 的核心思想：**预测下一个词时，不需要看前面所有的词，只需要看它前面的 $N-1$ 个词就够了。**

N-gram的优点是实现简单、容易理解，在许多任务中效果不错。

> N-gram 最强大的地方在于，它不需要复杂的神经网络训练，只需要**在海量文本里“数数”（频数统计）**。
>
> 以 Bigram 计算 $P(\text{苹果} \mid \text{吃})$ 为例：
>
> $$P(\text{苹果} \mid \text{吃}) = \frac{\text{“吃 苹果” 连续出现的次数}}{\text{“吃” 出现的总次数}}$$
>
> 如果你的语料库里，“吃”字出现了 1000 次，其中“吃 苹果”出现了 300 次，那么这个概率就是 $300 / 1000 = 0.3$。



但当N较大时，会出现数据稀疏性问题。模型的参数空间会急剧增大，相同的N-gram序列出现的概率变得非常低，导致模型无法有效学习，模型泛化能力下降。此外，N-gram模型忽略了词之间的范围依赖关系，无法捕捉到句子中的复杂结构和语义信息。

> #### N-gram 的局限性与大模型的进化
>
> 既然 $N$ 越大，考虑的前文越长，准确度就越高，那为什么我们不把 $N$ 设为 10、20 甚至 100 呢？
>
> 1. **数据稀疏问题（Sparsity）：** 随着 $N$ 的增大，某些词组连续出现的概率会变成 0。比如“我 喜欢 吃 冰镇的 爽口的 富士 苹果”，在现实语料库里可能一次都没出现过，导致整句话的概率直接变成 0。
>
> 2. **长距离依赖无法解决：** > “虽然**小明**在国外留学了很多年，习惯了吃西餐，但他最爱吃的依然是妈妈做的**红烧肉**。”
>
>    要预测最后的“红烧肉”，需要前面的“小明”（相隔了 20 多个词）。N-gram 无法开到这么大的窗口，而当今大模型（LLM）所使用的 **Transformer 架构（Self-Attention 自注意力机制）**，本质上彻底打破了 $N$ 的限制，能够一眼看到上下文里所有的词，从而完美解决了长距离依赖问题。
...
> 这段话提到了这两种架构，在面试或看原理图时，可以通过“多对一”**和**“一对多”的形状来快速区分：
>
> ![1.Word2Vec](../../../assets/MachineLearningAssets/1.Word2Vec.png)
>
> **CBOW（连续词袋）：** 如左图所示，它是**拿着周围的词，去猜中间的词**。比如输入 `[“我”, “喜欢”, “?”, “苹果”]`，让模型猜中间是 `“吃”`。它把周围词的向量直接相加（SUM）或平均，因此丢失了上下文的语序信息。
>
> **Skip-Gram：** 如右图所示，它是**拿着中间的词，去猜周围的词**。输入 `“吃”`，让模型去预测周围可能出现 `“喜欢”` 和 `“苹果”`。
>
> **为什么 Skip-Gram 在大语料上更好？** 因为在大型语料库中，一个词（如“吃”）可以和无数种上下文搭配。Skip-Gram 迫使模型去尝试预测各种各样的上下文组合，相当于增加了训练的难度和数据样本量，因此学出来的向量泛化能力更强、质量更高。
...
> 文中提到 `“国王”和“王后”在向量空间中的位置会比较接近`。Word2Vec 最让人惊艳的地方不仅是接近，而是它们之间存在**线性的几何关系（向量代数）**：
>
> $$\vec{\text{国王}} - \vec{\text{男人}} + \vec{\text{女人}} \approx \vec{\text{王后}}$$
>
> 这意味着，Word2Vec 成功把人类语言中的“性别”（Gender）这个抽象概念，映射成了多维几何空间里的一个**固定方向和距离的位移**。这种神奇的现象证明了：**通过上下文数数，神经网络真的能学到人类社会的逻辑。**

Word2Vec模型也可以很好的泛化到未见过的词，因为它是基于上下文信息学习的，而不是基于词典。但由于CBOW/Skip-Gram模型是基于局部上下文的，无法捕捉到长距离的依赖关系，缺乏整体的词与词之间的关系，因此在一些复杂的语义任务上表现不佳。

> 这里有一个很重要的信息表现不佳例如——**多义词问题**。
>
> > 思考一个词：**“苹果”**。
> >
> > - 句子 A：“我今天买了一个**苹果**，很甜。”（水果）
> > - 句子 B：“今天**苹果**发布了新款 iPhone。”（科技公司）
>
> **在 Word2Vec 中，不论什么语境，“苹果”这个词永远只有唯一一个、雷打不动的向量表示。** 它是静态的。为了兼顾两种意思，Word2Vec 只能把这个向量学到“水果”和“科技公司”的中间地带，结果导致在两个语境下表达得都不够精准。
...
> Word2Vec 的查表（Look-up）是死板的，而 ELMo 生成词向量是**活的、现算出来的**。
>
> 当遇到一句话时，ELMo 的处理方式是：
>
> 1. 它不再查死表，而是**把整句话输入到一个“双向 LSTM”神经网络中**。
> 2. 这个双向 LSTM 会同时做两件事：
>    - **正向 LSTM：** 从左到右读句子，看目标词的“上文”；
>    - **反向 LSTM：** 从右到左倒着读句子，看目标词的“下文”。
> 3. 最后，ELMo 把正向和反向学到的特征拼在一起。

ELMo采用典型的两阶段过程: 第1个阶段是利用语言模型进行预训练; 第2个阶段是在做特定任务时, 从预训练网络中提取对应单词的词向量作为新特征补充到下游任务中。基于RNN的LSTM模型训练时间长, 特征提取是ELMo模型优化和提升的关键。

> **第一阶段：无监督预训练（Pre-train）** 让 ELMo 在几百 GB 的海量流浪文本（比如整个维基百科）里瞎晃悠，通过“猜下一个词”的任务，把双向 LSTM 练得很聪明。这个阶段**不需要人工标注数据**。
>
> **第二阶段：下游任务特征提取/微调（Fine-tune）** 如果你要做一个“判断电影评论是好评还是差评”的任务，你只需要把你的电影评论输入训练好的 ELMo，把 ELMo 实时算出来的动态词向量“提取”出来，作为特征喂给你的分类器。

ELMo模型的主要优势在于其能够捕捉到词汇的多义性和上下文信息，生成的词向量更加丰富和准确，适用于多种 NLP 任务。然而，ELMo模型也存在一些问题，如模型复杂度高、训练时间长、计算资源消耗大等。

> ELMo 依然有局限性,被后来的 BERT 和 GPT 超越:
>
> **LSTM 的串行诅咒（无法并行化）：** LSTM（长短期记忆网络）是一种循环神经网络（RNN）。它在处理句子时，必须像老奶奶数钱一样，**读完第 1 个词，才能读第 2 个词，再读第 3 个词**。 这意味着，哪怕你有 1 万张英伟达最好的显卡，由于这种“前后依赖”的数学结构，你也只能让一张卡在那里慢慢按顺序跑，其他卡闲着。这在今天动辄千亿参数的大模型时代，是绝对无法接受的。
